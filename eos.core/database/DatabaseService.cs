        ...
        private async Task<List<TableDescriptor>> ListTables()
        {
            ...
            if (dataReader.HasRows)
            {
                while (await dataReader.ReadAsync())
                {
                    ...
                    var table = dataReader.GetString(1);
                    results.Add(new TableDescriptor
                    {
                        Name = table,
                        Schema = schema
                    });
...
;";
                ...
                if (dataReader.HasRows)
                {
                    while (await dataReader.ReadAsync())
                    {
                        var sourceSchema = dataReader.GetString(0);
                        var sourceTable = dataReader.GetString(1);
                        var sourceColumn = dataReader.GetString(2);
                        var targetSchema = dataReader.GetString(3);
                        var targetTable = dataReader.GetString(4);
                        var targetColumn = dataReader.GetString(5);
                        results.Add(new ForeignKeyDescriptor
                        {
...
;";
            ...
            if (dataReader.HasRows)
            {
                while (await dataReader.ReadAsync())
                {
                    ...
                    var columnType = dataReader.GetString(1);
                    results.Add(new ColumnDescriptor
                    {
                        Name = columnName,
                        Type = columnType
                    });
        ...
        private async Task<InsertsData> CollectExportData(string profilePath, string id, bool dummyFiles)
        {
            ...
            data.MainTable = data.Profile.Tables
                .Where(o => o.IsEntryPoint)
                .Select(o => o.Name)
                .SingleOrDefault();
        ...
        {
            await using var conn = new SqlConnection(this.ConnectionString);
            var query = $"select {table.LookupProperty} from {table.Name} where [{idProperty}] = @id;";
            var cmd = new SqlCommand(query, conn);
            ...
            if (dataReader.HasRows)
            {
                while (await dataReader.ReadAsync())
                {
                    var lookupColumnValue = dataReader.GetString(0);
                    substitute =
                        $"(select {idProperty} from {table.Name} where {table.LookupProperty} = '{lookupColumnValue}')";
                }
            }
        ...
        private string GetVariableName(TableDescriptor tableDescriptor, string id, InsertsData data, bool isBeingDefined)
        {
            ...
            if (!data.IdToVariable.TryGetValue(id, out variableName))
            {
                ...
                variableName =
                    $"@{table.Substring(0, 1).ToLower()}{table.Substring(1)}{data.RowsCountPerTable[table]}";
        ...
        private async Task CollectExportDataImplementation(TableDescriptor tableDescriptor, string id, bool dummyFiles, InsertsData data)
        {
            ...
            var table = tableDescriptor.ToString();
            var primaryKeyVariableName = GetVariableName(tableDescriptor, id, data, true);
            var pkColumn = await this.GetPrimaryKey(tableDescriptor);
            var foreignKeys = await this.ListForeignKeys();
            var columns = await this.ListColumns(tableDescriptor);
            var columnsForSql = string.Join(',', columns.Select(c => $"[{c.Name}]"));
            var referencedVariables = new List<string>();

            var outboundForeignKeys = foreignKeys
                .Where(o => o.Source.TableDescriptor.ToString() == table)
                .ToDictionary(o => o.Source.ColumnName, o => o);

            await using (var conn = new SqlConnection(this.ConnectionString))
            {
                var query = $"select {columnsForSql} from {table} where [{pkColumn}] = @id";
                var cmd = new SqlCommand(query, conn);
                cmd.Parameters.Add(new SqlParameter("id", id));
                    ...
                    while (await dataReader.ReadAsync())
                    {
                        ...
                        foreach (var column in columns)
                        {
                            ...
                            if (column.Name == pkColumn &&
                                data.IdToVariable.TryGetValue(dataReader.GetString(index), out var pKey))
                            {
                                sb.Append(pKey);
                                ++index;
                                continue;
                            ...
                            switch (column.Type)
                            {
                                case "nvarchar":
                                case "varchar":
                                    var stringValue = dataReader.GetValue(index).ToString();
                                    var hasForeignKeyToExportableTable = outboundForeignKeys.TryGetValue(column.Name, out var foreignKey);
                                    if (hasForeignKeyToExportableTable && data.Profile.Tables.Any(o =>
                                        o.Name == foreignKey.Target.TableDescriptor.ToString() && o.Export))
                                    {
                                        var isFollowingForeignKeyOk = data.Profile.ForeignKeys.Any(o =>
                                        ...
                                        var variableName = GetVariableName(foreignKey.Target.TableDescriptor, stringValue, data, false);
                                    ...
                                    else
                                    {
                                        ...
                                        if (hasForeignKeyToExportableTable)
                                        {
                                            ...
                                            {
                                                var replacementForId =
                                                    await GetLookupSql(referencedTable, foreignKey.Target.ColumnName,
                                                        stringValue);
                        ...
                        data.Inserts.Add(new InsertInfo
                        {
                            Sql = sb.ToString(),
            ...
            var inboundForeignKeys = foreignKeys
                .Where(o => o.Target.TableDescriptor.ToString() == table)
                .ToList();

            foreach (var inboundForeignKey in inboundForeignKeys)
            {
                if (data.Profile.Tables.Any(o =>
                        o.Name == inboundForeignKey.Source.TableDescriptor.ToString() && !o.IsEntryPoint && o.Export) &&
                    data.Profile.ForeignKeys.Any(o =>
                        o.Name == inboundForeignKey.Source.ToString() && o.Follow && !o.GoToParent))
                {
                    var pkOtherTable = await this.GetPrimaryKey(inboundForeignKey.Source.TableDescriptor);
                    await using var conn = new SqlConnection(this.ConnectionString);
                    var query =
                    ...
                    if (dataReader.HasRows)
                    {
                        while (await dataReader.ReadAsync())
                        {
                            var otherId = dataReader.GetString(0);
        ...
        private async Task CreateNewProfile(string profilePath)
        {
            ...
            {
                Tables = (await this.ListTables()).Select(o => new Table
                {
                    Name = $"[{o.Schema}].[{o.Name}]",
                    Export = false
                }).ToList(),
                ForeignKeys = (await this.ListForeignKeys()).Select(o => new ForeignKey
                {
                    Name =
                        $"[{o.Source.TableDescriptor.Schema}].[{o.Source.TableDescriptor.Name}].[{o.Source.ColumnName}]",
                    Follow = true,
                    GoToParent = false
                }).ToList()
            };

            await File.WriteAllTextAsync(profilePath, JsonSerializer.Serialize(profile, new JsonSerializerOptions
            {
                WriteIndented = true
            }));
        }
        ...
        public async Task Export(string exportToPath, string profile, string entryPointId, bool dummyFiles)
        {
            ...
            foreach (var (id, alias) in data.IdToVariable)
            {
                await file.WriteLineAsync(
                    $"declare {alias} as varchar(255) = LOWER(CONVERT(varchar(255), NEWID())); -- formerly: '{id}';");
            }
            ...
            while (addedSomething && availableReferences.Count < data.Inserts.Count)
            {
                ...
                foreach (var insert in data.Inserts)
                {
                    ...
                    var missingReference = insert.ReferencedVariables.Any(referencedVariable =>
                        !availableReferences.Contains(referencedVariable));
                    if (missingReference)
                    {
            ...
            if (availableReferences.Count < data.Inserts.Count)
            {
                ...
                foreach (var insert in data.Inserts)
                {
                    ...
                    var missingReferences = string.Join(',', insert.ReferencedVariables.Where(referencedVariable =>
                        !allReferences.Contains(referencedVariable)));
