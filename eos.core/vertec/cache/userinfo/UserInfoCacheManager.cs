namespace eos.core.vertec.cache.userinfo
{
    ...
    public class UserInfoCacheManager: VertecKeylessCacheManager<UserInfo>
    {
        ...
        public UserInfoCacheManager(Configuration configuration, VertecRequestsManager vertecRequestsManager) : base(configuration, "user info")
        {
                ...
                foreach (XmlNode node in projektBearbeiter.ChildNodes)
                {
                    ...
                    switch (propertyTag.Name)
                    {
                        case "eintrittPer":
                            return new UserInfo
                            {
                                FirstWorkDay = propertyTag.InnerText
                            };
                    }
                }
            }
