namespace eos.core.vertec.cache.presence
{
  ...
  public class PresenceCacheManager : VertecCacheManager<PresenceCacheKey, List<Presence>>
  {
    ...
    public PresenceCacheManager(Configuration configuration, VertecRequestsManager vertecRequestsManager)
      : base(configuration, "presence")
    {
    ...
    public override PresenceCacheKey GetCacheKey(string fileName)
    {
      ...
      return new PresenceCacheKey { Year = int.Parse(match.Groups[1].Value), Month = int.Parse(match.Groups[2].Value) };
    }
    ...
    public override async Task<List<Presence>> LoadDataRemotely(PresenceCacheKey cacheKey)
    {
      ...
      string timeRestriction =
        $"(von >= encodeDate({cacheKey.Year},{cacheKey.Month},1)) and (von < encodeDate({nextMonth.Year},{nextMonth.Month},1))";
      var request = new VertecRequest
      {
        Ocl = $"projektBearbeiter->select(loginName = '{this._vertecConfig.User}').praesenzzeiten->select({timeRestriction})",
        Members = new[] { "von", "bis", "text" },
