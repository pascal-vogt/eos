namespace eos.core.vertec.cache.worklog
{
  ...
  public class WorkLogCacheManager : VertecCacheManager<WorkLogCacheKey, List<WorkLogEntry>>
  {
    ...
    public WorkLogCacheManager(Configuration configuration, VertecRequestsManager vertecRequestsManager)
      : base(configuration, "work logs")
    {
    ...
    public override WorkLogCacheKey GetCacheKey(string fileName)
    {
      ...
      return new WorkLogCacheKey { Year = int.Parse(match.Groups[1].Value), Month = int.Parse(match.Groups[2].Value) };
    }
    ...
    private async Task<List<WorkLogEntry>> GetEntriesInternal(string collectionName, DateTime rangeStart, DateTime rangeEnd)
    {
      string timeRestriction =
        $"(datum >= encodeDate({rangeStart.Year},{rangeStart.Month},{rangeStart.Day})) and (datum <= encodeDate({rangeEnd.Year},{rangeEnd.Month},{rangeEnd.Day}))";
      var request = new VertecRequest
      {
        Ocl = $"projektBearbeiter->select(loginName = '{this._vertecConfig.User}').{collectionName}->select({timeRestriction})",
        Members = new[] { "datum", "text", "minutenint", "creationDateTime" },
        Expressions = new[] { new KeyValuePair<string, string>("projekt", "projekt.code"), new KeyValuePair<string, string>("phase", "phase.code") },
      };
